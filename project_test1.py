...
def spawn_object():
    ...
    # 🎯 Find tile where objects will spawn
    spawn_z = -32.0
    spawn_tile = None
    ...
    # 🚫 Prevent wide hole if tile already has lane holes
    if r < 0.05:
        if spawn_tile:
            holes = spawn_tile.get("lane_holes", [False, False, False])
            if any(holes):
                return  # ❌ don't spawn wide hole on broken ground

        objects.append({
            "lane": 1, "x": 0.0,
            "z": spawn_z, "kind": "wide_hole",
            "alive": True, "anim": 0.0,
            "ai_timer": 0.0,
        })
        return
    ...
    for lane in lanes[:count]:
        ...
        # 🚫 SKIP lane if hole exists there
        if spawn_tile:
            holes = spawn_tile.get("lane_holes", [False, False, False])
            if holes[lane]:
                continue   # ❌ NOTHING spawns here


        kind = choose_for_difficulty(lane_difficulty[lane])

        # small chance to force carrot
...
def draw_hud():
  ...
  if game_state == "playing":
      ...
      draw_text(16, WIN_H-152, f"Power Time: {max(player['inv_timer'], player['dj_timer'], player.get('speed_boost_timer',0.0)):.1f}s", 0.0, 0.85, 1.0)
    ...
    # 🎉 MAIN WIN TEXT
    draw_text(cx+40, WIN_H//2+70, " Crunch crunch carrot lunch!!", 0.2, 1.0, 0.2)

    # 🥕 stats
    draw_text(cx+20, WIN_H//2+30, f"Final Score : {score}", 1.0, 0.9, 0.4)
    draw_text(cx+20, WIN_H//2+5,  f"Carrots     : {carrots}", 1.0, 0.6, 0.2)

    # 🎮 instruction
    draw_text(cx+30, WIN_H//2-40, "Press ESC to Exit", 0.7, 0.7, 0.7)
...
def collect_object(o):
  ...
  if o["kind"] == "carrot":
      ...
      player["speed_boost_amount"] = POTION_SPEED_BOOST
      spawn_particles(o["x"], obj_cy(o), o["z"], (0.0,0.9,1.0), 14)
  elif o["kind"] == "red_potion":
      player["magnet"] = True; player["magnet_timer"] = 12.0
      # Red potions also grant a slightly larger temporary speed boost
      player["speed_boost"] = True; player["speed_boost_timer"] = POTION_SPEED_DURATION
      # A bit stronger than the regular potion
      player["speed_boost_amount"] = POTION_SPEED_BOOST + 0.06
...
def check_collisions():
    ...
    for o in objects:
        ...
        if collides_with_player(o):
            ...
            # 🐰 BIG ENEMY (fighter bunny)
            elif o.get("kind") == "big_bunny":

                # ✅ Player attacks (press F)
                if player["kick_timer"] > 0:
                    o["alive"] = False
                    score += 10
                    carrots += 10
                ...
                # ❌ Player fails → penalty
                    ...
                    # Do not automatically kill the big bunny when the player
                    # collides without attacking. The player must press 'F'
                    # (kick) to tackle it. Apply penalty but keep the enemy alive.
                    if carrots < 0:
                        carrots = 0
...
def update_game(dt):
   ...
   if player.get("speed_boost", False):
       amt = player.get("speed_boost_amount", POTION_SPEED_BOOST)
       game_speed *= (1.0 + amt)
